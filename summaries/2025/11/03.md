...
## 8:46:57 PM
**File:** `c:\Users\Dennis\Documents\projects\ambulensi\ambulensi-server\src\incident\incident.service.ts`
**Timestamp:** `11/3/2025, 11:01:15 AM`

This file defines the `IncidentService`, a core component for managing incidents within the `ambulensi-server` application. The service is highly integrated, utilizing numerous other services such as `AmbulanceService`, `TripService`, `TraccarService`, `BoundariesService`, `WebsocketGateway`, `TaskService`, `UploadFileService`, and interacting with `Incident` and `Patient` Mongoose models.

Key updates and features in this version include:

*   **Comprehensive Data Population:** The `populateFields()` method specifies an extensive list of related entities to be fetched with each incident, including patient details, ambulance information (with nested device, health facility, and crew details), health facilities, geographical boundaries (county, constituency, ward), user, organization, and PCR report data. This indicates a rich and deeply interconnected data model.
*   **Incident Creation (`create` method):**
    *   It now handles patient data creation or update before incident creation.
    *   Assigns geographical boundary IDs based on incident data.
    *   **New Feature:** Integrates with `TaskService` to create and assign tasks to both the assigned driver and EMT if they are present, linking the task directly to the incident.
    *   Emits real-time updates via `websocketGateway.emitIncidentUpdatesToSocket` after successful creation.
*   **Incident Update (`update` method):**
    *   Similarly handles patient data updates.
    *   **New Feature/Refinement:** Calls `syncAmbulanceCache()` after an incident update, suggesting cache invalidation or synchronization logic related to ambulances.
    *   Also emits real-time updates through the WebSocket gateway.
*   **Incident Reporting (`createIncidentReport` method):**
    *   Provides functionality to add incident reports, including handling multiple file attachments.
    *   Utilizes `uploadFileService` to upload files to Google Cloud Storage under the `INCIDENT_ATTACHEMENTS` folder and stores their URLs.
    *   Includes robust error handling for file uploads and incident not found scenarios.
*   **Search Functionality (`searchIncident` method):**
    *   Allows searching incidents by `searchTerm`, which is first used to find matching patients by name.
    *   Filters incidents based on `roleName` and `organizationId`, and then by the `_id`s of the matching patients.
*   **Data Retrieval:** Multiple getter methods (`getIncidentClassData`, `getIncidentCategoryData`, etc.) are present to expose static incident classification data, including specific categories and types for non-emergency incidents.
*   **Filtering and Pagination:** The `findAll` method supports role-based and organization-based filtering, query object parsing, and pagination, along with counting total and closed incidents. Similar filtering is applied to `getIncidentsByOrganizationId` and `getIncidentsByUserId`.

**Patterns and Recurring Elements:**
*   **Real-time Updates:** A consistent pattern is the emission of incident updates via `websocketGateway.emitIncidentUpdatesToSocket` after any significant change (creation, update), indicating a strong emphasis on real-time communication to clients.
*   **Dependency Injection:** The service heavily relies on dependency injection, demonstrating a modular and extensible architecture.
*   **Data-driven Classifications:** Incident data is categorized and classified using external data files (e.g., `INCIDENT_CLASS_DATA`, `INCIDENT_CATEGORY_DATA`), making the system configurable and consistent.
*   **Role and Organization-based Access:** Filtering by `roleName` and `organizationId` is a recurring theme across multiple query methods, suggesting a robust access control and multi-tenancy implementation.
*   **Error Handling:** Common NestJS exceptions (`BadRequestException`, `NotFoundException`, `InternalServerErrorException`) are used throughout the service for error management.